// Product Details Page - Individual product view with add to cart functionality
...
const ProductDetails = () => {
    ...
    // Fetch product from API or backend (tries both sources)
    const fetchProduct = async () => {
        try {
            ...
            if (!data || data.message === 'Product not found' || data.id === undefined) {
                // Try backend for custom product
                res = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/products/clothing`)
                const backendData = await res.json()
                if (backendData.success) {
                    const found = backendData.products.find(p => p._id === id)
                    if (found) {
                        ...
                        setProduct({
                            ...
                            thumbnail: `${import.meta.env.VITE_SERVER_URL}${found.image}`,
                            images: [`${import.meta.env.VITE_SERVER_URL}${found.image}`],
                            reviews: []
                        })
                        setSelectedImage(`${import.meta.env.VITE_SERVER_URL}${found.image}`)
                        setLoading(false)
                        return
                    }
                }
